var url=rootPath+ "/basicdata/notice", gridQryUrl = url+"/getUserNotice.json",custParame=SYSTEM.custParame,
status_=["未读","已读"];
	...
	loadGrid:function() {
		function t(val, opt, row) {
			var html_con = '<div class="operating" data-id="'
					+ row.id
					+ '"><span class="fa fa-eye mrb" title="查看"></div>';
			return html_con;
		}
		function fmtAmt(v) {
		...
		$("#grid").jqGrid({
			...
			colModel:[ {
				...
				align:"center",
				width:200,
				formatter:function(v){return status_[v];},
				sortable:true,
				title:true
			},{
				name:"create_time",
