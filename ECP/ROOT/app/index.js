...
var model=avalon.define({
	...
	Public.ajaxPost(rootPath+"/sso/user/loginInfo.json",
					...
					function(json) {
						...
						/** 获取自定义参数 */
						Public.ajaxPost(rootPath+"/basicdata/parame/list.json",{},function(json) {SYSTEM.custParame=json;});
						/**地区参数*/
						...
						/**未读通知*/
						Public.ajaxPost(rootPath+"/basicdata/notice/getUnrealNotice.json",{},function(json) {
							if(json.data){
								model.messageCount=json.data.length;
								//$('#msg').css('color','red');
							}
						});
					});
	...
	tab.addTabItem({
		tabid:"index",
		text:"通知",
		url:rootPath+"/basicdata/notice/noticeindex.html",
		showClose:true,
	});
...
window.onload = function(){
	if((screen.width==1366||screen.width==1360)&&screen.height==768){
		document.getElementById("container").style.minHeight="100px";
		document.getElementById("container").style.maxHeight="640px";
