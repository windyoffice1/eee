...
var model = avalon.define({
	...
	data:{id:"",from_warehouse_id:"",from_warehouse_name:"",target_warehouse_id:"",target_warehouse_name:"",transfer_total_money:"",operate_user_id:"",operate_user_name:"",company_id:"",
		operate_date:"",comment:"",
		productlistlength:1,order_amt:0,
		productlist:[{id:"",inventory_transfer_id:"",material_data_id: "",amount:0,target_price:0,total_money:0,comment:"",unit:"",material_data_name: "",unit:"",existing_amount:0,
			material_data_no:"",model_number:""}]},
	total_amount:0,
	transfer_total_money:0,
    warehouseList:[],
    qryWarehouseList:function(v){
    	Public.ajaxPost(rootPath+"/basicdata/warehouselocation/dataGrid.json",{keyword:v,rows:99999999,_sortField:"warehouse_name",_sort:"asc"},function(json){
    		model.warehouseList=json.data.list;
    		if(json.data.list!=null&&json.data.list.length>0){
    			console.info(json);
    			model.data.from_warehouse_id=json.data.list[0].id;
    		}
    	});
    },
    //添加商品
    ...
    qryProduct:function(v,idx){//自动完成查商品
    	...
    	Public.ajaxPost(rootPath+"/inventorymanager/transfer/getWarehouseMaterial.json",{material_no:v,id:model.data.from_warehouse_id},function(json){
...
function jisuan(value,index){//计算合计 注意：对于数组不能使用$watch 因为只能监听length。
	...
	model.data.transfer_total_money=model.total_money;
...
function postData(){
	...
	Public.ajaxPost(rootPath+"/inventorymanager/transfer/save.json",model.data.$model
			, function(t) {
		if (200 == t.status) {
