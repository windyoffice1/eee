var url=rootPath+"/returnmaterial/returnmaterialapply",gridQryUrl=url+"/dataGrid.json",
approve_status=["未提交","待审核","已批准","驳回","已取回"];
var model = avalon.define({$id:'view',
	query:{returnmaterial_no:"",returnmaterial_name:"",approve_status:"",is_deleted:0,},
	fastQryText:"快速查询",
	fastQry:[
	           {text:"清空条件 ",sl:false},
		       {text:"未提交",sl:false},
	...
	resetQry:function(){
		model.query={returnmaterial_no:"",returnmaterial_name:"",approve_status:"",is_deleted:0,};
		model.reloadData();
	},
	loadGrid:function() {
				function t(val, opt, row) {
					var html_con = '<div class="operating" data-id="'+ row.id+'" data-total_money="'+ row.total_money+'" data-child_warehouse_id="'+ row.child_warehouse_id+'"><span class="fa fa-eye mrb" title="查看"></span>';
	...
	addEvent:function() {
		...
		$(".grid-wrap").on("click", ".subAudit", function(e) {
			...
			if (Business.verifyRight("TD_UPDATE")) {
				...
				var child_warehouse_id = $(this).parent().data("child_warehouse_id");
				model.subAudit(t,total_money,child_warehouse_id);
			}
		});
	...
	del:function(id) {
		$.dialog.confirm("删除的退料申请单将不能恢复，请确认是否删除？", function() {
	...
	subAudit:function(id,total_money,child_warehouse_id) {
			Public.ajaxPost(url+"/subAudit.json", {id:id,total_money:total_money,child_warehouse_id:child_warehouse_id,status:1}, function(t) {
				if (t && 200 == t.status) {
					parent.Public.tips({type:2,content:t.msg});
					model.reloadData();
				} else{
