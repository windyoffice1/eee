...
//		filter.put("type",this.getParaToInt("type",-1));//0：供应商1：企业客户 2：个人客户
	...
	@PowerBind(code="A8_1_V",funcName="查看")
	public void qryOp() {
		getId();
	...
	@PowerBind(code="A8_1_E",funcName="编辑")
	public void save() {
		try {
			...
			po.set("updater_id", this.getUserMap().get("uid"));
			po.set("update_datetime", spdf.format(new Date()));
	...
	/**恢复*/
	@PowerBind(code="A8_1_E",funcName="恢复")
	public void reply() {
		try {
			getId();
	...
	/**放入客户池*/
	@PowerBind(code="A8_1_E",funcName="放入客户池")
	public void toPool() {
		try {
			getId();
	...
	/**领取1 或分配2*/
	@PowerBind(code="A8_1_E",funcName="分配或领取")
	public void allot() {
		try {
			getId();
	...
	/**导出*/
	@PowerBind(code="A8_1_O",funcName="导出")
	public void expout(){
		String company_id=this.getCompanyId();
		Map<String,Object> filter=new HashMap<String,Object>();
		filter.put("keyword",this.getPara("keyword"));
	...
	@PowerBind(code="A8_1_O",funcName="导入")
