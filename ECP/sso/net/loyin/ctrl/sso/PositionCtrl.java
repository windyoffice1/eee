	...
	public void save() {
		try {
			...
			if (StringUtils.isEmpty(id)) {
				...
				if(org!=null&&org.get("id").equals(po.get("parent_id"))){
					ChildWarehouse childWarehouse=new ChildWarehouse();
					childWarehouse.set("id", po.get("id"));
					childWarehouse.set("child_warehouse_name", po.get("name"));
					childWarehouse.set("company_id", this.getCompanyId());
					childWarehouse.set("type", 0);
					childWarehouse.save();
				}
			} else {
				//更新子公司
