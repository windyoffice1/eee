...
public class Position extends Model<Position> {
	...
	public void del(String id,String company_id) {
		if (StringUtils.isNotEmpty(id)){
			...
			Db.update("delete  from basic_child_warehouse where id in ("+ids_.toString()+") and company_id=?",parame.toArray());
		}
	}
	/**
	...
	//根据职位获取此职位上的人员id
	public List<String> getUseridByPositionName(String positionName){
		List<String> useridList= new ArrayList<String>();
		StringBuffer sql=new StringBuffer();
		sql.append("SELECT id FROM sso_user WHERE position_id in ")
			.append("(SELECT id from sso_position where name=? )");
		useridList = Db.query(sql.toString(), positionName);
		return useridList;
		
	}
	//根据组织名为“分公司”的组织
	public Position getPositionByOrgName(String orgName){
		String sql="SELECT * FROM sso_position WHERE name=? ";
		return dao.findFirst(sql,orgName);
	}
	//根据部门id获取此部门的上级部门
	public Position getSuperDepartment(String positionId){
		String sql="SELECT * FROM sso_position WHERE id=(SELECT parent_id FROM sso_position WHERE id=? )";
		Position position = dao.findFirst(sql, positionId);
		position.set("id", positionId);
		return position;
	}
}
