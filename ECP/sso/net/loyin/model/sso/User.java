...
public class User extends Model<User> {
	...
	public User findById(String id,String company_id){
		return User.dao.findFirst("select  u.*,ps.realname,ps.email,ps.mobile,ps.sex,ps.head_pic,ps.telephone,ps.address,p.name as position_name,d.name as department_name,d.id as department_id from " + tableName + " u, "+Position.tableName+" p ,"+Position.tableName+" d ,"+Person.tableName+" ps where  u.id=ps.id and d.id=p.department_id and u.position_id=p.id and u.id=? and u.company_id=?",id,company_id);
	}
	/**更新登录信息*/
	...
	/**获取用户的上级*/
	public List<User> findSupUser(String uid) {
		return find("select u.* from "+tableName+" u,"+Position.tableName+" p,"+tableName+" u1 where p.id=u1.position_id and u1.id=? and u.position_id=p.parent_id ",uid);
	}
	/**检查密码*/
	...
	/**获取用户的上级*/
	public List<User> findChildWarehouseManager(String child_warehouse_id) {
		return find("SELECT * FROM sso_user WHERE id=(SELECT manage_user_id FROM basic_child_warehouse WHERE id=? ) ",child_warehouse_id);
	}
	
	/**
	 * 设备部门经理和副经理 
	 */
	public List<User> findManagerOfEquipmentDepartment() {
		StringBuffer sql=new StringBuffer();
		sql.append("SELECT * FROM sso_user WHERE position_id IN (SELECT id FROM sso_position WHERE department_id in (SELECT id FROM sso_position WHERE name='设备部') ");
		sql.append("AND (name='经理' OR name='副经理'))");
