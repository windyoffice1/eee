...
public class ProcessCtrl extends AdminBaseController {
	...
	public void qryOp(){
		...
		if(model != null) {
			String JsonString =SnakerHelper.getModelJson(model);
			System.err.println(JsonString);
			jsonMap.put("process", JSON.parse(JsonString));
		}
	...
	/**
 	...
 	public void orderview(){
 		...
 		if(StringUtils.isBlank(orderid)){
 			this.rendJson(false, null, "获取流程异常！orderid不能为空");
 			break;
 		}
 		...
 		if(process!=null){
 			...
 			if(processModel != null) {
 				...
 				this.rendJson(false, null, "获取流程异常！");
 			}
 		}else{
 			setAttr("process", "");
 		}
 		...
 		//String jsonMap = SnakerHelper.getStateJson(processModel, tasks, historyTasks);
 		setAttr("active", jsonMap);
 		this.rendJson(true,null,"",jsonMap);
